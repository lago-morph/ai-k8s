...
class TestFileIOReadConfigFile:
    ...
    def test_read_config_file_with_malformed_lines(self, temp_config_file: Path) -> None:
...
class TestFileIOWriteConfigFile:
    ...
    def test_write_config_file_has_correct_content(self, temp_config_file: Path) -> None:
    ...
    def test_write_config_file_has_secure_permissions(self, temp_config_file: Path) -> None:
    ...
    def test_write_config_file_overwrites_existing(self, temp_config_file: Path) -> None:
...
class TestFileIOEnsureConfigDirectory:
    ...
    def test_ensure_config_directory_creates_directory(self, temp_config_dir: Path) -> None:
    ...
    def test_ensure_config_directory_does_not_fail_if_exists(self, temp_config_dir: Path) -> None:
...
class TestFileIOCheckFilePermissions:
    ...
    def test_check_file_permissions_returns_true_for_0600(self, temp_config_file: Path) -> None:
    ...
    def test_check_file_permissions_returns_false_for_insecure(self, temp_config_file: Path) -> None:
...
class TestFileIOProperties:
    ...
    @given(
        config=st.dictionaries(
            ...
            values=st.text(min_size=1, max_size=100, alphabet=st.characters(min_codepoint=33, max_codepoint=126)),  # Exclude space (32)
            min_size=1,
            max_size=10,
        )
    )
